...
- **黄色枠** = AIが作業中のカード（未完了の change に担当セッションが記録されている、または `owner` が `claude`）
- **水色枠** = あなたのアクションが必要なカード（完成確認待ち、担当があなた、今日の判断に掲載）
- 枠の色は上から順に決まります。担当セッションが記録されているカードは黄色枠、そうでなく水色枠の条件を満たせば水色枠、どちらでもなく `owner` が `claude` なら黄色枠です。`owner` の値だけでは水色枠を打ち消しません
- **担当セッションが記録されているカード**では「進める」ボタンが押せません。どの change を作業中かがカードのモーダルに出ます。`owner` が `claude` なだけのカードでは押せます
