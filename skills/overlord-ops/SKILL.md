...
- Merge a change yourself only through `change.sh merge <change-id>`, and only when all three of these hold: the change has been reviewed and its `reviewed_sha` is the pull request head, the pull request's CI passed (a pull request with no check at all counts as not verified), and the pull request's base is the working branch the card is built on. The command checks all three itself and refuses otherwise; do not work around a refusal.
- **Never merge into `main` or `master`, or into the repository default branch.** That merge releases the card and is the user's decision alone. It applies to the card-level delivery pull request `change.sh deliver` opens: create it, report it, and leave it for the user. Do not call `gh pr merge` directly for any pull request — `change.sh merge` is the only merge path, because it is the only one that checks the base branch.
