...
No agent edits this record by hand, and nothing else writes it: a delivery does not touch the card's `state`, and a failed delivery does not move the card out of `done`. It can write `changes`, whatever its outcome: step 1 of a delivery is the same synchronization `change sync <card-id>` performs, so a change whose pull request GitHub now reports differently has its `changes[].pr` rewritten, and one GitHub reports as merged also moves to `changes[].state: done` (nothing is written when no change moved). That can happen on every path - `created`, `updated`, `skipped`, `blocked` and `failed` alike - because the synchronization runs before the outcome is decided. A run that had nothing to deliver (`skipped`) or that found unmerged changes (`blocked`) writes no `delivery` at all, so those two outcomes leave whatever `delivery` the card already had, or none. See [the console reference](console.md) for every outcome and the event they are reported on.
